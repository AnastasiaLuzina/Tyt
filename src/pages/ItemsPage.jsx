...
import { fetchMyItems, fetchMyRequests } from '../api';

const ItemsPage = () => {
  const { telegramId } = useAuth();
  const [activeTab, setActiveTab] = useState('give'); // 'give' or 'want'
  ...
  const selectClaimer = (user) => {
    ...
    // Здесь можно вызвать API для выбора получателя
  };
                ...
                <div key={item.id} className="item-card">
                  ...
                  <div className="item-info">
                    ...
                    <div className={`item-status ${item.status === 'reserved' ? 'reserved' : 'active'}`}>
                      {item.status === 'reserved' ? 'Выбран человек' : 'Активно'}
                    </div>
